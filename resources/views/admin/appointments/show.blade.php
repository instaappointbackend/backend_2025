        ...
        <div class="col-lg-8">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                            ...
                            <div class="d-flex align-items-center">
                                <div class="me-3">
                                    <img src="{{ $appointment?->client?->profile_picture ? asset('storage/' . $appointment->client->profile_picture) : asset('admin/images/default-avatar.png') }}"
                                        alt="{{ $appointment?->client?->name }}" class="rounded-circle" width="50"
                                        height="50">
                                </div>
                                <div>
                            ...
                            <div class="d-flex align-items-center">
                                <div class="me-3">
                                    <img src="{{ $appointment->user->profile_picture ? asset('storage/' . $appointment->user->profile_picture) : asset('admin/images/default-avatar.png') }}"
                                        alt="{{ $appointment->user->name }}" class="rounded-circle" width="50"
                                        height="50">
                                </div>
                                <div>
                    ...
                    @if (
                        !empty($appointment->service->description) ||
                            (!empty($appointment->comboService) && !empty($appointment->comboService->description)))
                        <div class="mb-4">
                            <h6 class="text-muted mb-2">Service Description</h6>
                            <p>{{ !empty($appointment->service->description) ? $appointment->service->description : $appointment->comboService->description }}
                            </p>
                        </div>
                        <hr>
                    @endif
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        @if ($appointment->status == 'pending')
                            <form
                                action="{{ route('admin.appointments.status', ['appointment' => $appointment->id, 'status' => 'confirmed']) }}"
                                method="POST">
                                @csrf
                                @method('PATCH')
                        ...
                        @if ($appointment->status == 'confirmed')
                            <form
                                action="{{ route('admin.appointments.status', ['appointment' => $appointment->id, 'status' => 'completed']) }}"
                                method="POST">
                                @csrf
                                @method('PATCH')
                        ...
                        @if ($appointment->status == 'pending' || $appointment->status == 'confirmed')
                            <form
                                action="{{ route('admin.appointments.status', ['appointment' => $appointment->id, 'status' => 'cancelled']) }}"
                                method="POST">
                                @csrf
                                @method('PATCH')
                ...
                <div class="card-body">
                    @if ($appointment->payment_amount)
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <span
                                class="fw-bold">{{ $appointment->formatted_original_price ?? '₹' . number_format($appointment->original_price ?? 0, 2) }}</span>
                        </div>
                        ...
                        <div class="d-flex justify-content-between fw-bold">
                            ...
                            <span
                                class="fs-5">{{ $appointment->formatted_payment_amount ?? '₹' . number_format($appointment->payment_amount, 2) }}</span>
                        </div>

                        @if ($appointment->payment_method)
                            <div class="mt-3">
                                <h6 class="text-muted mb-2">Payment Method</h6>
                                <p>{{ ucfirst($appointment->payment_method) }}
                                    ({{ ucfirst($appointment->payment_mode ?? 'online') }})</p>
                            </div>
                        @endif
