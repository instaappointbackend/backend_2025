                ...
                <div class="alert alert-info">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            @endif
                            @if (request('start_date'))
                                <span class="badge bg-primary me-1">From:
                                    {{ \Carbon\Carbon::parse(request('start_date'))->format('M d, Y') }}</span>
                            @endif
                            @if (request('end_date'))
                                <span class="badge bg-primary me-1">To:
                                    {{ \Carbon\Carbon::parse(request('end_date'))->format('M d, Y') }}</span>
                            @endif
                            @if (request('vendor_id'))
                                <span class="badge bg-primary me-1">Vendor:
                                    {{ $vendors->find(request('vendor_id'))->name ?? 'Unknown' }}</span>
                            @endif
                            @if (request('client_id'))
                                <span class="badge bg-primary me-1">Customer:
                                    {{ $clients->find(request('client_id'))->name ?? 'Unknown' }}</span>
                            @endif
                            @if (request('service_id'))
                                <span class="badge bg-primary me-1">Service:
                                    {{ $services->find(request('service_id'))->name ?? 'Unknown' }}</span>
                            @endif
                        </div>
                    ...
                    <tbody>
                        @forelse($appointments as $index=>$appointment)
                            <tr>
                                ...
                                <td>{{ !empty($appointment->service->name) ? $appointment->service->name : $appointment->comboService->name }}
                                </td>
                                <td>
                                ...
                                <td colspan="8" class="text-center py-4">
                                    ...
                                    @if (request()->anyFilled(['search', 'status', 'start_date', 'end_date', 'vendor_id', 'client_id', 'service_id']))
                                        <a href="{{ route('admin.appointments.index') }}"
                                            class="btn btn-sm btn-outline-primary">Clear Filters</a>
                                    @endif
                                </td>
                            </tr>
                        @endforelse
