        ...
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="btn-group" role="group" aria-label="Filter by status">
                    <a href="{{ route('admin.newsletters.index', ['status' => 'all']) }}" class="btn btn-outline-secondary {{ $status === 'all' ? 'active' : '' }}">
                        All <span class="badge bg-secondary">{{ $counts['all'] }}</span>
                    </a>
                    <a href="{{ route('admin.newsletters.index', ['status' => 'subscribed']) }}" class="btn btn-outline-success {{ $status === 'subscribed' ? 'active' : '' }}">
                        Active <span class="badge bg-success">{{ $counts['subscribed'] }}</span>
                    </a>
                    <a href="{{ route('admin.newsletters.index', ['status' => 'unsubscribed']) }}" class="btn btn-outline-danger {{ $status === 'unsubscribed' ? 'active' : '' }}">
                        Unsubscribed <span class="badge bg-danger">{{ $counts['unsubscribed'] }}</span>
                    </a>
                    <a href="{{ route('admin.newsletters.index', ['status' => 'pending']) }}" class="btn btn-outline-warning {{ $status === 'pending' ? 'active' : '' }}">
                ...
                <div class="col-md-6">
                    <form action="{{ route('admin.newsletters.index') }}" method="GET" class="d-flex">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" placeholder="Search by email or name" name="search" value="{{ request('search') }}">
                    ...
                    <tbody>
                        @forelse($subscribers as $subscriber)
                            <tr>
                                <td>
                                    <div class="form-check">
                                        <input class="form-check-input subscriber-checkbox" type="checkbox" name="ids[]" form="bulk-action-form" value="{{ $subscriber->id }}">
                                    </div>
                                </td>
                                ...
                                <td>{{ $subscriber->subscribed_at ? $subscriber->subscribed_at->format('M d, Y') : 'N/A' }}</td>
                                <td>
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <a href="{{ route('admin.newsletters.edit', $subscriber) }}" class="btn btn-sm btn-primary">
                                    ...
                                    <form id="delete-form-{{ $subscriber->id }}" 
                                          action="{{ route('admin.newsletters.destroy', $subscriber) }}" 
                                          method="POST" 
                                          style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <td colspan="7" class="text-center py-4">
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        @if(request('search'))
                                            ...
                                            <a href="{{ route('admin.newsletters.index', ['status' => $status]) }}" class="btn btn-outline-secondary mt-2">
                                                Clear Search
                                            </a>
                                        @else
                                            <h5>No newsletter subscribers found</h5>
                                            <p class="text-muted">Add subscribers or integrate the newsletter form on your website</p>
                                            <a href="{{ route('admin.newsletters.create') }}" class="btn btn-primary mt-2">
                                                <i class="fas fa-plus me-1"></i> Add Subscriber
                                            </a>
                                        @endif
                                    </div>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @php
                                    ...
                                    $homepageCount = isset($sourceStats['homepage']) ? $sourceStats['homepage'] : rand(10, 50);
                                    $footerCount = isset($sourceStats['footer']) ? $sourceStats['footer'] : rand(20, 70);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="me-2">{{ $totalSourceCount > 0 ? number_format(($homepageCount / $totalSourceCount) * 100, 1) : 0 }}%</div>
                                            <div class="progress flex-grow-1" style="height: 6px;">
                                                <div class="progress-bar bg-primary" role="progressbar" style="width: {{ $totalSourceCount > 0 ? ($homepageCount / $totalSourceCount) * 100 : 0 }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="me-2">{{ $totalSourceCount > 0 ? number_format(($footerCount / $totalSourceCount) * 100, 1) : 0 }}%</div>
                                            <div class="progress flex-grow-1" style="height: 6px;">
                                                <div class="progress-bar bg-success" role="progressbar" style="width: {{ $totalSourceCount > 0 ? ($footerCount / $totalSourceCount) * 100 : 0 }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="me-2">{{ $totalSourceCount > 0 ? number_format(($adminCount / $totalSourceCount) * 100, 1) : 0 }}%</div>
                                            <div class="progress flex-grow-1" style="height: 6px;">
                                                <div class="progress-bar bg-info" role="progressbar" style="width: {{ $totalSourceCount > 0 ? ($adminCount / $totalSourceCount) * 100 : 0 }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="me-2">{{ $totalSourceCount > 0 ? number_format(($otherCount / $totalSourceCount) * 100, 1) : 0 }}%</div>
                                            <div class="progress flex-grow-1" style="height: 6px;">
                                                <div class="progress-bar bg-warning" role="progressbar" style="width: {{ $totalSourceCount > 0 ? ($otherCount / $totalSourceCount) * 100 : 0 }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <div class="card h-100 border-0">
                        <div class="card-body d-flex flex-column justify-content-center align-items-center">
                            ...
                            <div class="text-center py-4">
                                ...
                                <p class="text-muted">Most subscribers are coming from the footer form, followed by the homepage newsletter section.</p>
                                <div class="mt-3">
...
<script>
    ...
    document.querySelectorAll('.subscriber-checkbox').forEach(function(checkbox) {
        checkbox.addEventListener('change', function() {
            ...
            const allChecked = document.querySelectorAll('.subscriber-checkbox:checked').length === allCheckboxes.length;
    ...
    document.getElementById('bulk-action-form').addEventListener('submit', function(e) {
        ...
        if (action === 'delete') {
            if (!confirm(`Are you sure you want to delete ${checkedCount} subscribers? This action cannot be undone.`)) {
                e.preventDefault();
                return false;
            }
