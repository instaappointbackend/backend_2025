...
<a href="{{ route('admin.reports.appointments.export') }}?start_date={{ request('start_date') }}&end_date={{ request('end_date') }}&status={{ request('status') }}" class="btn btn-success me-2">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                ...
                <tr>
                    ...
                    <td>{{ !empty($appointment->service->name) ? $appointment->service->name : ($appointment->comboService->name ?? 'N/A') }}</td>
                    <td>{{ $appointment->client->name ?? 'N/A' }}</td>
                    <td>{{ $appointment->user->name ?? 'N/A' }}</td>
                    <td>{{ $appointment->date ? $appointment->date->format('M d, Y') . ', ' . \Carbon\Carbon::parse($appointment->start_time)->format('g:i A') . ' - ' . \Carbon\Carbon::parse($appointment->end_time)->format('g:i A') : 'N/A' }}</td>
                    <td>
                ...
                @empty
                <tr>
                    <td colspan="8" class="text-center">No appointments found in the selected date range.</td>
                </tr>
                @endforelse
                </tbody>
            </table>
