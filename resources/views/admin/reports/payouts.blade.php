...
@section('page-actions')
    <a href="{{ route('admin.reports.payouts.export') }}?start_date={{ request('start_date') }}&end_date={{ request('end_date') }}&status={{ request('status') }}"
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="2" class="text-center">No vendors found in the selected date range.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="me-2">
                                            <img src="{{ $payout->user && $payout->user->profile_picture ? asset('storage/' . $payout->user->profile_picture) : asset('admin/images/default-avatar.png') }}"
                                                alt="{{ $payout->user ? $payout->user->name : 'N/A' }}"
                                ...
                                <td>
                                    @if ($payout->bankAccount)
                                        ...
                                        {{ substr_replace($payout->bankAccount->account_number, 'XXXX', 0, -4) }}
                                    @else
                                        N/A
                                    @endif
                                </td>
                                ...
                                <td>{{ $payout->transaction_date ? $payout->transaction_date->format('M d, Y') : 'N/A' }}
                                </td>
                                <td>{{ $payout->created_at->format('M d, Y') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="text-center">No payout requests found in the selected date
                                    range.</td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            {{-- <div class="d-flex justify-content-end mt-3">
                {{ $payouts->onEachSide(5)->links() }}
            </div> --}}

            @if ($payouts->hasPages())
                <div class="d-flex justify-content-between align-items-center mt-3">
                    <div>
                        Showing {{ $payouts->firstItem() ?? 0 }} to {{ $payouts->lastItem() ?? 0 }} of
                        {{ $payouts->total() }}
                        payouts
                    </div>
                    <div class="d-flex justify-content-end mt-3">
