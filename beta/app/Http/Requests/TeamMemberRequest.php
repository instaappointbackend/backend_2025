...
class TeamMemberRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($this->isMethod('put') || $this->isMethod('patch')) {
            // Assume the team member's ID is provided as a route parameter named "id"
            $teamMemberId = $this->route('id');
            
            return [
                'name'     => 'sometimes|required|string|max:255',
                'mobile'   => 'sometimes|required|string|digits:10|unique:users,mobile' . $teamMemberId,
                'email'    => 'sometimes|required|email|unique:users,email,' . $teamMemberId,
