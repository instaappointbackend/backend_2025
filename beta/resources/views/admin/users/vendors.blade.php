                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="me-2">
                                            <img src="{{ $vendor->profile_picture ? asset('storage/' . $vendor->profile_picture) : asset('admin/images/default-avatar.png') }}" alt="{{ $vendor->name }}" class="avatar-img" width="40" height="40">
                                        </div>
                                        <div>
                                            <h6 class="mb-0">{{ $vendor->name }}</h6>
                                            @if(isset($vendor->businessDetail) && $vendor->businessDetail)
                                                <small class="text-muted">{{ $vendor->businessDetail->business_name }}</small>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex">
                                        <a href="{{ route('admin.users.show', $vendor->id) }}" class="btn btn-sm btn-info me-1" data-bs-toggle="tooltip" title="View Details">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.users.edit', $vendor->id) }}" class="btn btn-sm btn-primary me-1" data-bs-toggle="tooltip" title="Edit Vendor">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.users.destroy', $vendor->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger" data-confirm="Are you sure you want to delete this vendor?" data-bs-toggle="tooltip" title="Delete Vendor">
