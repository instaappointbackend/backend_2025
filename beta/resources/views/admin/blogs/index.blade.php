                    ...
                    <tbody>
                        @forelse($blogs as $blog)
                            <tr>
                                <td>{{ $blog->id }}</td>
                                <td>
                                    <a href="{{ route('admin.blogs.show', $blog->id) }}" class="fw-bold text-decoration-none">
                                        {{ $blog->title }}
                                    </a>
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="me-2">
                                                @if($blog->user->profile_picture)
                                                    <img src="{{ asset('storage/' . $blog->user->profile_picture) }}" alt="{{ $blog->user->name }}" class="rounded-circle" width="30" height="30">
                                                @else
                                                    <div class="bg-primary rounded-circle d-flex align-items-center justify-content-center" style="width: 30px; height: 30px; color: white;">
                                ...
                                <td>
                                    <span class="badge bg-{{ $blog->status == 'published' ? 'success' : 'warning text-dark' }}">
                                        {{ ucfirst($blog->status) }}
                                    </span>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        <a href="{{ route('admin.blogs.show', $blog->id) }}" class="btn btn-sm btn-info" data-bs-toggle="tooltip" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.blogs.edit', $blog->id) }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="Edit">
