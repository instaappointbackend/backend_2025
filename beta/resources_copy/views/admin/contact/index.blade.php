...
<style>
    ...
    .stats-card.bg-info { background: linear-gradient(135deg, #36d1dc 0%, #5b86e5 100%); }
    .stats-card.bg-warning { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); }
    .stats-card.bg-success { background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); }
    .stats-card.bg-primary { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); }
    .stats-card.bg-secondary { background: linear-gradient(135deg, #f2f2f2 0%, #d4d4d4 100%); color: #333; }
    .stats-card.bg-danger { background: linear-gradient(135deg, #fa709a 0%, #fee140 100%); }
            ...
            <div class="row align-items-end">
                ...
                <div class="col-md-2">
                    ...
                    <select name="source" class="form-control">
                        ...
                        <option value="users" {{ request('source') === 'users' ? 'selected' : '' }}>Registered Users</option>
                    ...
                    @if($contacts->count() > 0)
                                ...
                                <tbody>
                                    @foreach($contacts as $contact)
                                        <tr class="{{ $contact->status === 'unread' ? 'bg-light-warning' : '' }}">
                                            <td>
                                                <div class="custom-control custom-checkbox">
                                                    <input type="checkbox" class="custom-control-input contact-checkbox" id="contact-{{ $contact->id }}" value="{{ $contact->id }}">
                                                    <label class="custom-control-label" for="contact-{{ $contact->id }}"></label>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <div class="ticket-priority-indicator bg-{{ $contact->status === 'unread' ? 'warning' : ($contact->status === 'replied' ? 'success' : 'info') }}"></div>
                                            ...
                                            <td>
                                                <div class="user-info">
                                                    ...
                                                    <div class="user-details ml-2">
                                                        <div class="font-weight-bold">{{ $contact->getSenderName() }}</div>
                                                        <small class="text-muted">{{ $contact->getSenderEmail() }}</small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="subject-cell">
                                                    <a href="{{ route('admin.contacts.show', $contact) }}" class="text-decoration-none text-dark font-weight-medium">
                                            ...
                                            <td>
                                                <span class="ticket-badge badge-{{ $contact->status === 'unread' ? 'warning' : ($contact->status === 'replied' ? 'success' : ($contact->status === 'spam' ? 'danger' : 'info')) }}">
                                                    @if($contact->status === 'unread')
                                                        <i class="fas fa-envelope mr-1"></i>
                                            ...
                                            <td>
                                                <div class="date-info">
                                                    <div class="font-weight-medium">{{ $contact->created_at->format('M d, Y') }}</div>
                                            ...
                                            <td>
                                                <div class="action-buttons">
                                                    ...
                                                    @if($contact->status !== 'replied')
                                                        <form method="POST" action="{{ route('admin.contacts.update-status', $contact) }}" style="display: inline;">
                                                            @csrf
                                                            @method('PUT')
                        ...
                        <div class="bulk-actions">
                            <div class="row align-items-center">
                                <div class="col-md-6">
                                    <form method="POST" action="{{ route('admin.contacts.bulk-action') }}" id="bulk-action-form" class="d-flex align-items-center">
                                        @csrf
                                        <label class="font-weight-bold mr-3">Bulk Actions:</label>
                                        <select name="action" class="form-control mr-3" style="width: auto;" required>
