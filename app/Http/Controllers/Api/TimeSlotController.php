...
class TimeSlotController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //     // The entered date is today
        //     $timeSlots = TimeSlot::where('user_id', $userId)
        //         ->where('date', $date)
        //         ->where('end_time', '>', Carbon::now()->format('H:i:s')) // ✅ only future slots
        //         ->orderBy('start_time')
        //         ->get();
        // } else if (Carbon::parse($date)->gt(Carbon::today())) {
        //     $timeSlots = TimeSlot::where('user_id', $userId)
        //         ->where('date', $date)
        //         //->where('end_time', '>', Carbon::now()->format('H:i:s')) // ✅ only future slots
        //         ->orderBy('start_time')
        ...
        if ($serviceId || $comboServiceId) {
            ...
            //                 if (
            //                     $nextSlotStartTime->eq($nextSlotTime) &&
    ...
    public function generate(Request $request)
    {
        ...
        // $interval = 0; // Default 0 minutes
        // $bufferTime = 0; // Default 0 minutes
        $userId = Auth::id();
